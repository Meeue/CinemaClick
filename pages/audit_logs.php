...
<div style="background:var(--accent-dim);border:.5px solid var(--accent);border-radius:10px;padding:14px 18px;margin-bottom:20px;display:flex;align-items:center;gap:14px">
  ...
  <div>
    <div style="font-size:13px;font-weight:600;color:var(--text)">Audit Log — Auto-generated by MySQL Triggers</div>
    <div style="font-size:12px;color:var(--text-muted);margin-top:3px">
      Read-only. Every INSERT, UPDATE, and DELETE across all tables is logged automatically.
      Reads from <strong>SLAVE</strong> DB. Never editable from the UI.
    </div>
  </div>
</div>
...
<div class="table-wrap"><table>
  <thead><tr><th>Log ID</th><th>Operation</th><th>Table</th><th>Record ID</th><th>Old Data</th><th>New Data</th><th>Timestamp</th><th style="text-align:right">Detail</th></tr></thead>
  <tbody>
...
function viewLog(l){
  ...
  document.getElementById('lgBody').innerHTML =
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px">' +
      ...
      vf('Record ID',   l.record_id||'—') +
      vf('Timestamp',   l.changed_at) +
      vf('Changed By',  l.changed_by||'system') +
    '</div>' +
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">' +
      '<div><div style="font-size:11px;font-weight:700;color:var(--danger);text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">Before (Old Data)</div>' +
      ...
      '<div><div style="font-size:11px;font-weight:700;color:var(--success);text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">After (New Data)</div>' +
